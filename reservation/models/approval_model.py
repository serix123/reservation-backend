...
class Approval(models.Model):
    ...
    admin_status = models.IntegerField(default=0, choices=APPROVER_CHOICE)
    ...
    def check_if_approved(self):
        with transaction.atomic():
            if (
                self.immediate_head_status
                and self.person_in_charge_status
                and self.admin_status
            ):
                ...
                self.immediate_head_status == -1
                or self.person_in_charge_status == -1
                or self.admin_status == -1
            ):
