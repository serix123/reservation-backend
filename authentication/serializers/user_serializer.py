...
class ResidentRegistrationSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        Residence.objects.create(
            user=user, role="Resident", registration_date=timezone.now().date()
        )
        return user
...
class StaffRegistrationSerializer(serializers.ModelSerializer):
    password = serializers.CharField(write_only=True)
    role = serializers.ChoiceField(choices=["Admin", "Officer"], write_only=True)
...
class AdminUserUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if self.instance == self.context["request"].user:
            raise serializers.ValidationError("You cannot modify your own permissions")
        return attrs
